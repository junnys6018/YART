...
namespace yart
{
	...
	// Given a unit vector v1, constructs an orthonormal set { v1, v2, v3 }. Note this set is only unique up to rotation about v1
	template <typename T>
	...
	template <typename T>
	Vector3<T> Lerp(Float t, const Vector3<T>& u, const Vector3<T>& v)
	{
		return (1 - t) * u + t * v;
	}
	...
	template <typename T>
	Vector2<T> Lerp(Float t, const Vector2<T>& u, const Vector2<T>& v)
	{
		return (1 - t) * u + t * v;
	}

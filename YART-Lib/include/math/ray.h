...
namespace yart
{
    ...
    {
    public:
        Ray() : m_Tmax(Infinity), m_Time(0)
        {
        }

        Ray(const Vector3f& o, const Vector3f& d, real tmax = Infinity, real time = 0) : o(o), d(d), m_Tmax(tmax), m_Time(time)
        {
        }
    ...
    public:
        ...
        RayDifferential(const Ray& ray) : Ray(ray), m_HasDifferentials(false)
        {
        }
